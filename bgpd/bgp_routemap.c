...
struct route_map_rule_cmd route_set_evpn_router_mac_cmd = {
	"extcommunity evpn rmac", route_set_evpn_router_mac,
	route_set_evpn_router_mac_compile, route_set_evpn_router_mac_free};
...
DEFUN_YANG (set_ecommunity_evpn_rmac,
...
{
	const char *xpath =
		"./set-action[action='frr-bgp-route-map:set-extcommunity-evpn-rmac']";
	char xpath_value[XPATH_MAXLEN];

	nb_cli_enqueue_change(vty, xpath, NB_OP_CREATE, NULL);

	snprintf(xpath_value, sizeof(xpath_value),
		 "%s/rmap-set-action/frr-bgp-route-map:extcommunity-evpn-rmac",
		 xpath);
...
DEFUN_YANG (no_set_ecommunity_evpn_rmac,
