...
/*
 * XPath: /frr-route-map:lib/route-map/entry/match-condition/rmap-match-condition/frr-bgp-route-map:nexthop-fanout
 */
int lib_route_map_entry_match_condition_rmap_match_condition_nexthop_fanout_modify(
	struct nb_cb_modify_args *args)
{
	struct routemap_hook_context *rhc;
	const char *fanout;
	...
	case NB_EV_APPLY:
		/* Add configuration. */
		rhc = nb_running_get_entry(args->dnode, NULL, true);
		fanout = yang_dnode_get_string(args->dnode, NULL);

		/* Set destroy information. */
		rhc->rhc_mhook = bgp_route_match_delete;
		rhc->rhc_rule = "nexthop-fanout";
		rhc->rhc_event = RMAP_EVENT_MATCH_DELETED;

		ret = bgp_route_match_add(rhc->rhc_rmi, "nexthop-fanout",
					  fanout, RMAP_EVENT_MATCH_ADDED,
					  args->errmsg, args->errmsg_len);
...
int lib_route_map_entry_match_condition_rmap_match_condition_nexthop_fanout_destroy(
	struct nb_cb_destroy_args *args)
{
	switch (args->event) {
	case NB_EV_VALIDATE:
